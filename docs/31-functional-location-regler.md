...
1. Den kører testmatrixen (nedenfor, 95 sager) gennem **originalen** og
...
|---|---|---|---|---|---|---|
| FL1 | FL normaliseres: NBSP→mellemrum, CR/LF fjernes, VERSALER, trim | `app-functional-location.js:2368`, `fl-rule-engine.js:395` | – | Afledt | `inpFlRowFl.OnChange`: `Upper(Trim(Substitute(Substitute(Substitute(Self.Text, Char(160), " "), Char(13), ""), Char(10), "")))` | `fl_parts.py` `norm_fl()` → `inpFlRowFl.OnChange` |
| FL2 | FL og Description højst 40 tegn i inputfeltet | `:1508`, `:1511` (`maxlength="40"`) | – | Afledt | `MaxLength: 40` | `fl_parts.py` `inpFlRowFl`, `inpFlRowDesc` |
| FL3 | Tom række (FL og Description tomme efter trim) er *Draft*: valideres ikke, kommer ikke i klassefanerne, gemmes ikke | `:1076-1083`, `:1479`, `:2278` | – | Afledt | `IsBlank` i `colFlCalc`; `Status = "draft"`; filtreret fra i gem | `fl_validation.py` `calc_rows()`; `fl_save.py` `SAVE_ROWS` |
| FL4 | FL mangler (Description udfyldt) | `:1085-1087` | `FL required.` | Fejl | Rækkebesked Ord 1 | `fl_validation.py` `row_msgs()` |
| FL5 | FL dublet - ALLE rækker med samme normaliserede FL | `:1047-1057`, `:1089-1091` | `Duplicate FL.` | Fejl | `CountRows(Filter(colFlRows, FL = fl)) > 1`, Ord 2 | `fl_validation.py` `row_msgs()` |
| FL6 | Description mangler | `:1093-1095` | `Description required before Ready for SAP.` | Fejl | Ord 3 | `fl_validation.py` `row_msgs()` |
| FL7 | Description over 40 tegn | `:1097-1099` | `Description > 40.` | Fejl | Ord 4 | `fl_validation.py` `row_msgs()` |
| FL8 | KKS-syntaks: FL skal matche et af 12 mønstre. KKS-type: `KKSKV` (mønster 11-12), `KKSKA` (10), ellers `KKS`; de tre giver syntaksklassen `KAB` | `fl-rule-engine.js:6-19, 55-99` | – | Afledt | 12 × `IsMatch` med konstante mønstre | `fl_validation.py` `syntax()` |
| FL9 | Legacy-nummerering (`…[-A-Z]{2}\d{1,3}$`) uden gyldigt mønster: KKS-type `KKS` | `fl-rule-engine.js:21, 66-72` | `Legacy format.` | Advarsel | `IsMatch(fl, LEGACY)` → advarsel | `fl_validation.py` `syntax()` |
| FL10 | Intet mønster og ikke legacy | `fl-rule-engine.js:74-78` | `KKS invalid.` | Fejl | Ord 5 | `fl_validation.py` `row_msgs()` |
| FL11 | Pos. 1-3 skal være et værk i `Plant` (AVV, ASV, HEV, KYV, SKV, SSV, HCV, SMV) | `fl-rule-engine.js:126-128, 267-270` | `Plant key invalid.` | Fejl | `in nfFlPlants.Key`, Ord 6 | `fl_validation.py` `row_msgs()`; `generate_app_onstart.py` `nfFlPlants` |
| FL12 | Pos. 7-9 skal findes i `FunctionKeyDict` - **kun hvis ordbogen ikke er tom** | `fl-rule-engine.js:130-132` | `Function key invalid.` | Fejl | Opslag i `MD_FLKey` (delegeret `LookUp`), sprunget over når `nfFlHasFunctionKeys` er falsk, Ord 7 | `fl_validation.py` `FK_LOOKUP`, `row_msgs()` |
| FL13 | Aggregat (pos. 12-13) `UF`/`UE`: funktionsnøglen skal starte med `U` | `fl-rule-engine.js:243-249` | `UF/UE requires U function key.` | Fejl | Ord 8 | `fl_validation.py` `row_msgs()` |
| FL14 | `UF`/`UE` uden BR18-regelsæt | `:251-255` | `BR18 rules missing for <key12>.` | Fejl | `IsBlank(LookUp(nfFlBr18, Key12 = k12))`, Ord 9 | `fl_validation.py` `row_msgs()` |
| FL15 | Pos. 17-18 skal være tilladt for aggregatet (`UE`: FP/FD/FG/FH, `UF`: D/G/H) | `:257-259` | `BR18 key17 invalid for <key12>.` | Fejl | Ord 10 | `fl_validation.py` `row_msgs()` |
| FL16 | Syntaksklasse `KAB` giver klassen `KAB` - nøgle 12/18 tjekkes ikke | `fl-rule-engine.js:138-142` | – | Afledt | Første gren i `cls` | `fl_validation.py` `klasse()` |
| FL17 | Pos. 18-19 har to tegn: både komponent (`ClassDeterminationComponentKey`) og aggregat skal findes; klassen er komponentens | `:144-155, 167-198` | `Component key invalid.` / `Equipment key invalid.` | Fejl | `nfFlComponent`, `nfFlAggregate`, Ord 11-12 | `fl_validation.py` `klasse()`, `row_msgs()` |
| FL18 | Ellers: aggregatet giver klassen | `:209-213` | – | Afledt | `LookUp(nfFlAggregate, Key = k12).Cls` | `fl_validation.py` `klasse()` |
| FL19 | Ellers: MKP-/FP-mønster uden kendt aggregat | `:215-218` | `Equipment key invalid.` | Fejl | Ord 13 | `fl_validation.py` `row_msgs()` |
| FL20 | Ellers: fem korte mønstre → `NO CLASS` | `:220-225` | – | Afledt | 5 × `IsMatch` | `fl_validation.py` `klasse()` |
| FL21 | Ellers: `[A-B][A-Z]{2}\d{2}`-mønstret → `ELF` | `:227-231` | – | Afledt | `IsMatch` | `fl_validation.py` `klasse()` |
| FL22 | Ellers ingen klasse | `:233` | `Class not determined.` | Fejl | Ord 14 | `fl_validation.py` `row_msgs()` |
| FL23 | Ingen klasse og ingen fejl → `NO CLASS` | `app-functional-location.js:1115-1118` | – | Afledt | `If(cls = "" && CountRows(msgs) = 0, "NO CLASS", cls)` | `fl_validation.py` `calc_rows()` |
| FL24 | Regeldata ikke indlæst | `fl-rule-engine.js:115-118` | `Rules data missing.` | Fejl | `CountRows(nfFlAggregate) = 0`, Ord 0 | `fl_validation.py` `row_msgs()` |
| FL25 | Status: `invalid` ved fejl, `warning` ved advarsler, ellers `valid`. SAP-status `Error not ready for SAP` / `Ready for SAP` / `Draft` | `:1129-1135`, `:2199-2203` | – | Afledt | `Status` i `colFlRows` | `fl_validation.py` `STATUS`; `fl_parts.py` `txtFlRowStatus` |
| FL26 | Valideringskolonnen viser første fejl, ellers første advarsel; statuschippen skjules ved en fejl | `:1547-1571` | – | Afledt | `FirstIssue`/`FirstWarning` | `fl_parts.py` `txtFlRowIssue`, `txtFlRowStatus` |
| FL27 | Hjælpetekst pr. besked (nøglens position og værdi) | `:1573-1630` | Ordret, fx `Function key is FL position 7-9 (<key7>). It must exist in FunctionKeyDict lookup.` | Afledt | `Tooltip` på `txtFlRowIssue` | `fl_validation.py` `hint_fx()`; `fl_parts.py` |
| FL28 | Klassefaner: rækker, der ikke er draft og har en klasse. `ALL (n)` + én fane pr. klasse, alfabetisk; rækker sorteret på FL; klasse-hjælpetekst som tooltip | `:1475-1495`, `:1632-1665`, `:2353-2357` | – | Afledt | `galFlTabs`, `galFlClassRows` | `fl_parts.py` `build_classes()` |
| FL29 | Feltmarkering: FL rødt ved en besked med FL/KKS/Plant/Function/Component/Equipment/BR18/Class; Description ved "Description" | `:2284-2318` | – | Afledt | `BorderColor` på de to inputs | `fl_parts.py` `FL_BAD`, `DESC_BAD` |
...
| ID | Regel | Kilde | Besked (ordret) | Alvor | Power Fx | Implementeret i |
|---|---|---|---|---|---|---|
| FL30 | `Char`-tabellen: klassens felter med tal-`MaxLength` | `:806-828`, `:1183-1190` | `<felt>: Max N characters.` | Fejl | Tjek `MAX` i `nfFlPlan` | `fl_validation.py` `spool_issues()`; `generate_app_onstart.py` `nfFlPlan` |
| FL31 | `Char`-tabellen: `Dropdown`-felter skal matche listen | `:1192-1197` | `<felt>: Value must match dropdown options.` | Fejl | Tjek `DROP` | samme |
| FL32 | Description er påkrævet (stamdata) | `:203-213`, `:1236-1238` | `Description: Required field.` | Fejl | Tjek `REQ` | samme |
| FL33 | Stamdata-længder: Manufacturer 30, Description 40, Model Number 20, Manufacturer Part/Serial Number 30, Sort Field 30, Room 8, Warranty Start/End 10 | `:203-213`, `:1240-1242` | `<felt>: Max N characters.` | Fejl | Tjek `MAX` | samme |
| FL34 | Warranty Start/End: `DD.MM.YYYY` eller `YYYYMMDD`, og en rigtig dato | `:179`, `:1244-1246`, `:1440-1462` | `<felt>: Use DD.MM.YYYY or YYYYMMDD.` | Fejl | Tjek `DATE` - ren aritmetik, ikke `Date()`, se PX4 | `fl_validation.py` `DATE_OK` |
| FL35 | Faste værdier: Atex/Risiko/Asbestos/PTW = X, ABC Indic. = A, StrIndicator ∈ KKS/AKS/ROS/KKSKV/KKSKA, TRM assignment = X | `:149-157`, `:1249-1257` | `<felt>: Allowed values: X.` (listen ordret) | Fejl | Tjek `ALLOW` | `fl_validation.py` `spool_issues()` |
| FL36 | Trinliste pr. klasse (`CLASS_STEP_RULES`, `DEFAULT` for ukendte) | `:181-201`, `:1222-1229` | – | Afledt | Foldet ind i `nfFlPlan` af harnessen | `fl_rules.generated.json` → `nfFlPlan` |
| FL37 | `Design_pressure_`: Design pressure maks. **8**, tegn `0-9 . , - / +`; uom mod `Design_pressure_uom` | `:216-218`, `:1260-1263` | `Max 8 characters.` / `Invalid value format.` / `Value must match dropdown options.` | Fejl | `MAX`, `RGX NUMSIGN`, `DROP` | `fl_validation.py` `spool_issues()`, `RGX` |
| FL38 | `Operating_pressure_`: maks. 12, samme tegn; uom mod `Design_pressure_uom` | `:219-221`, `:1265-1268` | som FL37 | Fejl | samme | samme |
| FL39 | `Design_temperature_`: maks. 8; uom mod `Design_Temp_uom` | `:222-224`, `:1270-1273` | som FL37 | Fejl | samme | samme |
| FL40 | `Operating_temperature_`: maks. 10; uom mod `Design_Temp_uom` | `:225-227`, `:1275-1278` | som FL37 | Fejl | samme | samme |
| FL41 | `Design_flow_`: maks. 10; uom mod `Design_flow_uom` | `:228-230`, `:1280-1283` | som FL37 | Fejl | samme | samme |
| FL42 | I et trin: klassens `Dropdown`-felter med en kendt søgenøgle (K0210 …) tjekkes igen | `:1362-1371` | `Value must match dropdown options.` | Fejl | Falder sammen med FL31 (samme felt, samme liste - dedupliceret) | `fl_rules.generated.json` (dedupliceret) |
| FL43 | `Typekreds`: Typekredse mod `TypekredsTabel` | `:1285-1287` | `Typekredse: Value must match dropdown options.` | Fejl | `DROP` | `fl_validation.py` `spool_issues()` |
| FL44 | `TestMethod`: Test Method og Test Method 2 mod `TestMethod` | `:1289-1292` | `<felt>: Value must match dropdown options.` | Fejl | `DROP` | samme |
| FL45 | `TRMNEW`: EX-Marking maks. 30 | `:231-233`, `:1295` | `EX-Marking: Max 30 characters.` | Fejl | `MAX` | samme |
| FL46 | `TRMNEW`: Safety Critical Equipment mod klassens egen `SCEq`-kolonne (ingen kolonne = intet tjek) | `:879-902`, `:1297`, `:1405-1407` | `Safety Critical Equipment: Value must match dropdown options.` | Fejl | `DROP` mod klassens liste | samme |
| FL47 | `TRMNEW`, kun MKP og NO CLASS: Fire Classification mod `FireClassification`, Fire Sealing Type mod `Table20` | `:1299-1302` | `FIRE CLASSIFICATION: …` / `FIRE SEALING TYPE: Value must match dropdown options.` | Fejl | `DROP` | samme |
| FL48 | `TRMNEW`: udfyldt EX-Marking, SCE, Fire Classification, Fire Sealing Type eller Fire Sealing Product → TRM assignment = X og ABC Indic. = A. Ellers ryddes TRM assignment (ABC bliver). Sker **efter** tjekkene | `:1304-1313` | – | Afledt | Skrives i `colFlVals` efter meddelelsestabellen | `fl_validation.py` `TRM_SET` |
| FL49 | MKP og aggregat `UE`: TRM assignment = X | `:1315-1321` | – | Afledt | samme | samme |
| FL50 | MKP og `UE`: Fire Classification påkrævet | `:1323-1325` | `FIRE CLASSIFICATION: Required when aggregate key is UE.` | Fejl | `UE` | `fl_validation.py` `spool_issues()` |
| FL51 | MKP, `UE` og pos. 17-18 = `FP`: Fire Sealing Type og Product påkrævet | `:1327-1334` | `FIRE SEALING TYPE: Required for UE/FP.` / `FIRE SEALING PRODUCT: Required for UE/FP.` | Fejl | `UEFP` | samme |
| FL52 | Klassens egne felter (`CLASS_SPECIFIC_FIELD_RULES`, ELF … UNF): længde og tegn | `:236-341`, `:1339-1344`, `:1374-1388` | `<felt>: Max N characters.` / `<felt>: Invalid value format.` | Fejl | `MAX`, `RGX NUMCOMMA`/`NUMDOT` | samme |
| FL53 | `KKS_Syntax`-trinnet gentager syntaksfejlen på feltet | `:1346-1354` | `Functional Location: KKS invalid.` | Fejl | `KKS` | samme |
| FL54 | Listematch: uden forskel på store og små bogstaver, trimmet, `|`-adskilte værdier skal ALLE være i listen; tom liste = intet tjek | `:1426-1438` | – | Afledt | `Split(v, "|")` + `in` | `fl_validation.py` `in_list()` |
| FL55 | Beskedformat `<etiket>: <besked>`; identiske beskeder én gang; feltets første besked vises ved feltet | `:1166-1177`, `:1464-1473`, `:2157-2186` | – | Afledt | `GroupBy(RowGuid, Msg)` + `Min(Ord)` | `fl_validation.py` `DEDUPE` |
| FL56 | Værdier trimmes; en tom værdi fjerner feltet | `:582-606` | – | Afledt | `RemoveIf` + `Collect` kun ved ikke-tom | `fl_parts.py` `set_val_fx()` |
| FL57 | Afledte felter: StrIndicator = KKS-type, Long text = Description som standard, User status = status med versaler, System status = `LOCAL`, Info = første fejl/advarsel, SAP status | `:2079-2096`, `:2205-2242` | – | Afledt | `Switch` i `val_fx()` | `fl_parts.py` `display_val()` |
| FL58 | Skrivebeskyttede kolonner: Class, SAP status, Info, StrIndicator, Str. Indicator, User status, System status | `:115-123`, `:2137-2139` | – | Afledt | `Editable` i `nfFlColumns` | `generate_app_onstart.py` `nfFlColumns`; `fl_parts.py` |
| FL59 | Editor: dropdown når feltet har en liste (også de faste lister), ellers tekst med maks.-længde (Char, FL 40, Description 40). En ugyldig gemt værdi vises stadig | `:2098-2135`, `:2188-2197` | – | Afledt | `drpFlDetVal` / `inpFlDetVal`, `MaxLength` | `fl_parts.py` `build_detail()` |
| FL60 | Kolonnerne pr. klasse fra `FL_SPOOL_COLUMNS`. Kompakt visning: #, FL, Description, StrIndicator, Class, Info (SAP status skjult) | `:65-73`, `:1905-1958`, `:2036-2054` | – | Afledt | `nfFlColumns`; kompakt tabel | `fl_parts.py` `build_classes()` |
| FL61 | Detaljeruden: alle klassens kolonner med editor og besked; "Show empty"/"Hide empty" (standard: skjul tomme uden besked) | `:1960-2034` | – | Afledt | `galFlDetail`, `varFlShowEmpty` | `fl_parts.py` `build_detail()` |
| FL62 | Verify validerer alle rækker; ændring af FL/Description og af et spool-felt validerer igen | `:431-462`, `:570-580`, `:1014-1045` | – | Afledt | `Select(btnFlVerify)` i `OnChange` | `fl_parts.py`; `fl_validation.py` `verify_fx()` |
| FL63 | Add row; Delete row (sidste række væk → ny tom række); siden starter med én tom række | `:399`, `:423-429`, `:464-479`, `:917-935` | – | Afledt | `btnFlAddRow`, `btnFlRowDelete`, `OnVisible` | `fl_parts.py`, `assemble_screen.py` |
| FL64 | Export JSON: `generatedAt`, `source`, `rows`, `classBuckets`, `ruleMeta` | `:2254-2276` | – | Afledt | `JSON()` → `Download` af data-URI + vist i ruden | `fl_save.py` `EXPORT` |
| FL65 | Klassernes hjælpetekster (`CLASS_HELP`) | `:75-113` | – | Afledt | `nfFlClassHelp` | `generate_app_onstart.py` |
| FL66 | Tællere: rækker i alt, klar (valid+warning), med fejl | `:2244-2252` | – | Afledt | Badges i bjælken | `fl_parts.py` `build_bar()` |
| FL67 | Uden virkning i originalen: `FIELD_REGEX_RULES` bruges ingen steder; trinet `VerifyFunctionalLocationClasses` gør intet; `FL_CLASSIFICATION_DATA` overskrives af `FL_LOOKUPS` | `:159-177`, `:182-200`, `fl-rule-engine.js:264` | – | – | Bevidst ikke implementeret: det ville give en regel, originalen ikke har | – (efterprøvet af differentialtesten) |

### Regler, appen lægger til (fra opgaven, ikke fra HTML'en)

| ID | Regel | Kilde | Besked | Alvor | Implementeret i |
|---|---|---|---|---|---|
| FL68 | Indsend blokeres, så længe en ikke-tom række er `invalid`, der ingen klar række er, eller der er ændret noget siden sidste Verify | Opgaven, §3 "blokér indsend ved fejl"; HTML'ens "Ready for SAP" | `Submit is blocked: N row(s) have errors.` | Fejl | `fl_save.py` `SUBMIT_DM`, `SUBMIT` |
...
| PX5 | `FunctionKeyDict` har 6.445 rækker - over SharePoints delegeringsgrænse (500/2000) | Nøglerne ligger i `MD_FLKey` (indekseret `KeyType` + `KeyValue`) og slås op med én delegeret `LookUp` pr. **forskellig** nøgle 7 i Verify (`colFlFk`). Aggregat (146), komponent (139), BR18 (7) og værker (8) er små og ligger som navngivne formler |
...
| PX9 | Hovertooltips (`data-hint`) | Kontrollernes `Tooltip`-egenskab med samme tekst |
...
| FL62 | Ret FL → rækken valideres igen med det samme | – |
| FL63 | Slet eneste række → én ny tom række | – |
| FL64 | Export JSON → fil med `rows`, `classBuckets`, `ruleMeta` | – |
| FL66 | 3 rækker, 1 med fejl → `Rows: 3`, `Ready: 2`, `Issues: 1` | – |
| FL68 | Ingen fejl, Verify kørt → Submit aktiv | En række `invalid` → Submit inaktiv og beskeden vist |
| FL69 | Indsendt → felterne er grå | – |
...
| AQ7 | JS'en tjekker `Function key` kun, når ordbogen ikke er tom. I appen betyder en tom `MD_FLKey` (ikke provisioneret) derfor, at tjekket springes over | Samme adfærd som JS. Provisioneringen seeder listen, så det kun sker, hvis den ikke er kørt |
