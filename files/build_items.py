...
from gen_screen import Ctrl, C_CARD_BG, C_CARD_BORDER, C_TITLE, C_MUTED, C_REQUIRED, C_PRIMARY, C_WHITE, \
    C_INFO_FG, C_INFO_BG, C_VALID_FG, C_VALID_BG, C_INVALID_FG, C_INVALID_BG, C_NEUTRAL_FG, C_NEUTRAL_BG, FONT
...
def build_items_rail():
    ...
    btnRemove = button(
        ...
        (
            ...
            ")"
        ), danger=True, width=112)

    btnRow = group("conVhpItemButtonRow", [btnAdd, btnCopy, btnRemove], direction="Horizontal", gap=8, height=36,
                   wrap="true", align_items="Center")
    ...
    cardTextCol = group("conVhpItemCardText", [cardTitle, cardFl, cardTasklist], direction="Vertical", gap=2,
                        height=44, fill_portions=1, width="Parent.Width - 70 - 10")
    ...
    cardRight = group("conVhpItemCardRight", [cardStatus, btnOpen], direction="Vertical", gap=6, height=68,
                      align_items="End", width=70)

    itemCard = group(
        "conVhpItemCard", [cardTextCol, cardRight], direction="Horizontal", gap=10, height="Parent.TemplateHeight - 2",
        ...
        border_thickness=1, radius=10, pad=(10, 12, 10, 12), width="Parent.TemplateWidth", align_items="Center")
    ...
    gallery = Ctrl(
        ...
        props={
            ...
            "Fill": "RGBA(0, 0, 0, 0)",
            "FillPortions": "0",
            "Height": "IfError(Max(CountRows(colVhpItems), 1) * 88, 88)",
    ...
    card = group("conVhpItemsCard", [header, btnRow, gallery, emptyState], direction="Vertical", gap=12,
                 height="IfError(48 + 12 + 36 + 12 + Max(CountRows(colVhpItems), 1) * 88 + 12 + If(CountRows(colVhpItems) = 0, 40, 0), 240)",
                 fill=C_CARD_BG, border_color=C_CARD_BORDER, radius=14, pad=(18, 18, 18, 18),
                 width="If(App.Width < 1000, Parent.Width, 360)", fill_portions=1)
    return card
...
def build_item_editor():
    ...
    flBlock = group("conVhpItemFlBlock", [flLabelRow, flRow, flDescription, flMeta], direction="Vertical", gap=6,
                    height=20 + 36 + 18 + 18 + 18, width="Parent.Width")
    ...
    row2 = two_col_row("conVhpItemRow2",
                        ...
                        field_cell("conVhpCellItemObjList", "Object List", txtObjList))
    ...
    row4 = two_col_row("conVhpItemRow4",
                        ...
                        group("conVhpCellItemSpacer", [], height=62,
                              width="If(App.Width < 640, Parent.Width, (Parent.Width - 20) / 2)"))

    fieldsGrid = group("conVhpItemFieldsGrid", [row1, row2, row3, row4], direction="Vertical", gap=16,
                       height="If(App.Width < 640, 4 * (2 * 62 + 20) + 3 * 16, 4 * 62 + 3 * 16)")
    ...
    longTextCell = field_cell("conVhpCellItemLongText", "Item Long Text", txtLongText, height=80,
                              width="Parent.Width", fill_portions_formula="0")
    ...
    card = group(
        "conVhpEditorCard", [header, flBlock, fieldsGrid, longTextCell, footer], direction="Vertical", gap=14,
        height=48 + 14 + (20 + 36 + 18 + 18 + 18) + 14 + (4 * 62 + 3 * 16) + 14 + (20 + 6 + 80) + 14 + 36,
        fill=C_CARD_BG, border_color=C_CARD_BORDER, radius=14, pad=(18, 18, 18, 18), fill_portions=1,
        width="If(App.Width < 1000, Parent.Width, Parent.Width - 360 - 20)")
    return card
...
def build_items_section():
    ...
    split = group("conVhpItemsSplit", [rail, editor], direction="Horizontal", gap=20,
                  height="IfError(If(App.Width < 1000, conVhpItemsCard.Height + 20 + conVhpEditorCard.Height, Max(conVhpItemsCard.Height, conVhpEditorCard.Height)), 700)",
